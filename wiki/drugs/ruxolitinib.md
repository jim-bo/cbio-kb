---
name: ruxolitinib
targets:
  - JAK1
  - JAK2
drug_class: JAK1/2 inhibitor
canonical_source: corpus
unverified: true
tags:
  - targeted-therapy
  - jak-inhibitor
processed_by: wiki-cli
processed_at: 2026-05-16
---

# ruxolitinib

## Overview

Ruxolitinib is an oral JAK1/2 inhibitor FDA-approved for myelofibrosis, polycythemia vera, and steroid-refractory acute graft-versus-host disease. In sarcoma it has been explored in functional drug screens where JAK pathway activity may contribute to tumor growth.

## Evidence in the corpus

- In the UCLA PDTO sarcoma screen, rapidly progressing tumors (patients with progressive disease at follow-up) showed increased sensitivity to ruxolitinib (p=0.043) compared to non-rapidly-progressing tumors, suggesting that aggressive sarcoma biology may be associated with JAK pathway dependency. [PMID:39305899](../papers/39305899.md)
- Ruxolitinib (JAK1/2 inhibitor) was active against HUT78 CTCL cells harboring a [JAK3](../genes/JAK3.md) p.Ala573Val activating mutation; supports genotype-stratified use in JAK3/STAT3/SH2B3-mutant CTCL [PMID:26551667](../papers/26551667.md)
- In the Beat AML program ex vivo screen, co-mutation of [BCOR](../genes/BCOR.md) with [RUNX1](../genes/RUNX1.md) correlated with sensitivity to all four JAK inhibitors tested including ruxolitinib; BCOR alone or BCOR co-mutant with [DNMT3A](../genes/DNMT3A.md) or [SRSF2](../genes/SRSF2.md) did not show this sensitivity, confirming a strict co-mutational requirement [PMID:30333627](../papers/30333627.md).

## Resistance mechanisms

- Not reported in corpus.

## Cancer types (linked)

- [OS](../cancer_types/OS.md), [LMS](../cancer_types/LMS.md), [RMS](../cancer_types/RMS.md)

## Sources

- [PMID:39305899](../papers/39305899.md) — Duminuco et al. 2024, UCLA PDTO sarcoma functional screen; association with rapidly progressive disease phenotype.
- [PMID:26551667](../papers/26551667.md)
- [PMID:30333627](../papers/30333627.md)

*This page was processed by **wiki-cli** on **2026-05-16**.*
