---
symbol: SMARCA4
aliases: []
cancer_types: [LUAD, NSCLC]
tags: [chromatin-remodeler, metastasis, brain-metastasis, bone-metastasis]
processed_by: entity-page-writer
processed_at: 2026-04-08
---

# SMARCA4

## Overview

SMARCA4 encodes the ATPase subunit of the SWI/SNF chromatin remodeling complex. In the cbio-kb corpus it appears as a metastasis-associated driver in lung adenocarcinoma.

## Alterations observed in the corpus

- SMARCA4 alterations were strongly enriched in [LUAD](../cancer_types/LUAD.md) patients with bone metastasis (OR=6.47) and shortened time to bone metastasis (OR=2.79, 95% CI 1.71–4.56, p<0.001) [PMID:37084736](../papers/37084736.md).
- SMARCA4, [STK11](../genes/STK11.md), [KEAP1](../genes/KEAP1.md), and [KRAS](../genes/KRAS.md) alterations were mutually exclusive with [EGFR](../genes/EGFR.md) alterations in [LUAD](../cancer_types/LUAD.md) [PMID:37084736](../papers/37084736.md).
- Inactivation of SMARCA4 (together with [TP53](../genes/TP53.md) and [CDKN2A](../genes/CDKN2A.md)) correlated with site-specific shorter time to metastasis in [LUAD](../cancer_types/LUAD.md) [PMID:37084736](../papers/37084736.md).
- SMARCA4 alterations were enriched in primary LUAD tumors from patients who later developed brain metastasis compared with non-metastatic primaries [PMID:37591896](../papers/37591896.md).
- SMARCA4 was included in LUAD pathway/metastasis analyses in MSK-CHORD (n=24,950) [PMID:39506116](../papers/39506116.md).

## Cancer types (linked)

- [LUAD](../cancer_types/LUAD.md) — metastasis-associated driver with site-specific bone tropism [PMID:37084736](../papers/37084736.md); enriched in BM+ primaries [PMID:37591896](../papers/37591896.md).

## Co-occurrence and mutual exclusivity

- Mutually exclusive with [EGFR](../genes/EGFR.md) alterations in [LUAD](../cancer_types/LUAD.md) [PMID:37084736](../papers/37084736.md).

## Therapeutic relevance

- No direct targeted therapy reported in the corpus.

## Open questions

- Some site-specific analyses (e.g., liver) for SMARCA4 were underpowered [PMID:37084736](../papers/37084736.md).

## Sources

- [PMID:37084736](../papers/37084736.md)
- [PMID:37591896](../papers/37591896.md)
- [PMID:39506116](../papers/39506116.md)

*This page was processed by **entity-page-writer** on **2026-04-08**.*
