---
name: Pleomorphic Rhabdomyosarcoma
oncotree_code: PRMS
main_type: Soft Tissue Sarcoma
parent: RMS
tags:
  - rhabdomyosarcoma
  - soft-tissue-sarcoma
  - genomically-complex
  - corpus-shorthand
canonical_source: corpus
unverified: true
processed_by: crosslinker
processed_at: 2026-05-21
---

# Pleomorphic Rhabdomyosarcoma (PRMS)

## Overview

Pleomorphic Rhabdomyosarcoma is a high-grade rhabdomyosarcoma subtype occurring predominantly in adults, characterized by extensive cellular and nuclear pleomorphism without the canonical translocations of alveolar [RMS](../cancer_types/RMS.md) (PAX3/7–[FOXO1](../genes/FOXO1.md)). It is the rarest and most aggressive RMS subtype. **Note:** The canonical OncoTree code is PLRMS (Pleomorphic Rhabdomyosarcoma), not PRMS. The code "PRMS" is corpus shorthand used in the dataset `sarcoma_msk_2022`. The closest canonical page is [RMS](../cancer_types/RMS.md).

## Cohorts in the corpus

- [sarcoma_msk_2022](../datasets/sarcoma_msk_2022.md): PRMS was among the 44 sarcoma histologies profiled by FoundationOne Heme in 7,494 sarcomas [PMID:35705558](../papers/35705558.md).

## Recurrent alterations

- In a 7,494-sarcoma cohort, pleomorphic rhabdomyosarcoma was included as a distinct RMS subtype among the 44 histologies; genomically complex sarcomas showed p53 pathway (58.2%) and Rb pathway (56%) alteration; complex RMS variants lack the defining PAX3/7–FOXO1 fusions of alveolar RMS [PMID:35705558](../papers/35705558.md).

## Subtypes

- Pleomorphic RMS is distinguished from embryonal RMS ([ERMS](../cancer_types/ERMS.md)) and alveolar RMS ([ARMS](../cancer_types/ARMS.md)) by histologic pleomorphism and adult onset.

## Therapeutic landscape

- No established targeted therapy; treated with anthracycline-based sarcoma chemotherapy regimens. High genomic complexity relative to other RMS subtypes may predict ICI responsiveness in TMB-high cases.

## Sources

- [PMID:35705558](../papers/35705558.md) — Gounder et al. 2022, 7,494-sarcoma FoundationOne Heme cohort.

*This page was processed by **crosslinker** on **2026-05-21**.*
