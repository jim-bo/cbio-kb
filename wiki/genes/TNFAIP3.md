---
symbol: TNFAIP3
aliases: [A20]
cancer_types: [CHL]
tags: [nfkb, hodgkin-lymphoma]
processed_by: crosslinker
processed_at: 2026-05-21
---

# TNFAIP3

## Overview

TNFAIP3 (A20) is a negative regulator of NF-κB signaling and a recurrent driver in classic Hodgkin lymphoma.

## Alterations observed in the corpus

- TNFAIP3 was mutated in 36% of classic Hodgkin lymphoma cases — the second most common driver after [SOCS1](../genes/SOCS1.md) — and is an NF-κB pathway driver [PMID:36723991](../papers/36723991.md).
- Recurrently mutated in diffuse large B-cell lymphoma (DLBCL) and follicular lymphoma identified by whole-genome/exome sequencing [PMID:21796119](../papers/21796119.md)
- Identified as significantly mutated in TCGA [LUSC](../cancer_types/LUSC.md) (178 tumors) via COSMIC-restricted analysis [PMID:22960745](../papers/22960745.md)
- 6q23 deletion in 55% of primary central nervous system lymphoma ([PCNSL](../cancer_types/PCNSL.md)) cases; somatic mutations in 20%; part of the BCR/TLR/NF-kB pathway alterations present in >90% of [PCNSL](../cancer_types/PCNSL.md). [PMID:25991819](../papers/25991819.md)
- Tumor-suppressor mutational pattern in DLBCL [PMID:28985567](../papers/28985567.md)
- NF-kB pathway mutations including TNFAIP3 (A20) occur in DLBCL genetic cluster C1 (marginal-zone-like, NOTCH2/SPEN-enriched) alongside [BCL10](../genes/BCL10.md) and [FAS](../genes/FAS.md) apoptosis-mediator mutations [PMID:29713087](../papers/29713087.md)
- TNFAIP3 (A20) listed among recurrently altered genes in the MSK-IMPACT pan-sarcoma cohort of 2,138 bone and soft tissue tumors; alterations observed across multiple subtypes in the epigenetic/immune-regulatory axis [PMID:35705560](../papers/35705560.md)

## Cancer types (linked)

- [CHL](../cancer_types/CHL.md) — 36% mutated; NF-κB pathway driver [PMID:36723991](../papers/36723991.md).

## Co-occurrence and mutual exclusivity

- Part of the NF-κB driver set in cHL alongside [NFKBIE](../genes/NFKBIE.md) [PMID:36723991](../papers/36723991.md).

## Therapeutic relevance

- No direct targeted therapy reported in the corpus.

## Open questions

- None specific to TNFAIP3 in the corpus.

## Sources

- [PMID:36723991](../papers/36723991.md)
- [PMID:21796119](../papers/21796119.md)
- [PMID:22960745](../papers/22960745.md)
- [PMID:25991819](../papers/25991819.md)
- [PMID:28985567](../papers/28985567.md)
- [PMID:29713087](../papers/29713087.md)
- [PMID:35705560](../papers/35705560.md)

*This page was processed by **crosslinker** on **2026-05-21**.*
