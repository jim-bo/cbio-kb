---
name: LC-MS/MS proteomics
slug: lc-ms-ms-proteomics
kind: method
canonical_source: corpus
unverified: true
tags: [proteomics, mass-spectrometry, multi-omics]
processed_by: wiki-cli
processed_at: 2026-05-16
---

# LC-MS/MS proteomics

## Overview

Liquid chromatography tandem mass spectrometry (LC-MS/MS) whole-cell proteomics enables deep quantitative profiling of the tumor proteome. Single-shot LC-MS/MS can quantify thousands of proteins from a single tumor sample without extensive sample fractionation [PMID:34433969](../papers/34433969.md).

## Used by

- [PMID:34433969](../papers/34433969.md) — single-shot LC-MS/MS whole-cell proteomics was applied to 96 meningioma tumors from the University Health Network Brain Tumor BioBank, quantifying 6,568 proteins; integrated with WES, EPIC methylation, and mRNA-seq data to characterize four stable molecular groups (MG1–MG4) and identify MG4-specific proteomic markers [PMID:34433969](../papers/34433969.md).
- Applied to 15 neuroblastoma tumor samples to quantify protein abundance and stratify tumors by [MYCN](../genes/MYCN.md) amplification status [PMID:41063451](../papers/41063451.md)
- LC-MS/MS used for global proteome and phosphoproteome quantification in TCGA high-grade serous ovarian cancer samples [PMID:40694421](../papers/40694421.md)
- LC-MS/MS proteomics (PXD059301) used to identify USP10 as a TRMT10A protein interactor from Co-IP pulldown in prostate cancer cells [PMID:41071892](../papers/41071892.md).
- PWAS-inspired allele-specific proteomics (LC-MS/MS) on oligos carrying the rs4519489 A or T allele identified USF1, TBX3, and TFAP4 as A-allele-preferring binders; USF1 was the only one confirmed by ChIP-qPCR at the locus [PMID:41062477](../papers/41062477.md)
- Label-free shotgun LC-MS/MS applied to 100 CPTAC colon cancer tumors; average protein profiles correlated with TCGA colorectal at Pearson r=0.96; identified 31 proteins >2-fold elevated in tumors vs NATs [PMID:31031003](../papers/31031003.md)
- Core assay in the CCLE proteomics study (n=375 cancer cell lines, 22 lineages): TMT 10-plex LC-MS/MS on Thermo Orbitrap Fusion / Fusion Lumos (42 ten-plexes × 12 fractions = 504 LC-MS runs); >12,000 proteins quantified at 1% protein-level FDR [PMID:31978347](../papers/31978347.md).
- Label-free LC-MS/MS used for GC-MS metabolome and LC-MS lipidome profiling (582 lipids quantified across 75 of 99 GBM tumors) in the CPTAC GBM proteogenomic study [PMID:33577785](../papers/33577785.md)

## Notes

- Proteomics data complemented mRNA-seq to reveal post-transcriptional regulation differences across meningioma molecular groups [PMID:34433969](../papers/34433969.md).
- Corpus-grown slug; not present in canonical ontology.

## Sources

- [PMID:34433969](../papers/34433969.md)
- [PMID:41063451](../papers/41063451.md)
- [PMID:40694421](../papers/40694421.md)
- [PMID:41071892](../papers/41071892.md)
- [PMID:41062477](../papers/41062477.md)
- [PMID:31031003](../papers/31031003.md)
- [PMID:31978347](../papers/31978347.md)
- [PMID:33577785](../papers/33577785.md)

*This page was processed by **wiki-cli** on **2026-05-16**.*
