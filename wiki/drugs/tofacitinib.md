---
name: tofacitinib
targets:
  - JAK1
  - JAK3
drug_class: JAK inhibitor
canonical_source: corpus
unverified: true
tags:
  - targeted-therapy
  - jak-inhibitor
  - ctcl
processed_by: wiki-cli
processed_at: 2026-05-16
---

# tofacitinib

## Overview

Tofacitinib is a small-molecule inhibitor of [JAK1](../genes/JAK1.md) and [JAK3](../genes/JAK3.md) (with lesser activity against [JAK2](../genes/JAK2.md) and [TYK2](../genes/TYK2.md)), blocking downstream STAT signaling. It is FDA-approved for rheumatoid arthritis, psoriatic arthritis, ulcerative colitis, and ankylosing spondylitis. In oncology it has been investigated in hematologic malignancies driven by constitutive JAK-STAT pathway activation, including T-cell lymphomas with [JAK3](../genes/JAK3.md) mutations.

## Evidence in the corpus

- Tofacitinib (JAK1/3 inhibitor) was active against HUT78 CTCL cells harboring a JAK3 p.Ala573Val activating mutation alongside [ruxolitinib](../drugs/ruxolitinib.md); MEK1/2 inhibition and NFAT inhibition had only modest effects in the same lines, supporting genotype-stratified JAK inhibitor use in JAK3/STAT3/SH2B3-mutant CTCL [PMID:26551667](../papers/26551667.md)
- In the Beat AML program ex vivo screen, tofacitinib (JAK1/JAK3 inhibitor) was among the four JAK inhibitors to which [BCOR](../genes/BCOR.md)/[RUNX1](../genes/RUNX1.md) co-mutant AML specimens showed selective ex vivo sensitivity; the signal was absent in BCOR-solo or other BCOR co-mutation contexts [PMID:30333627](../papers/30333627.md).

## Resistance mechanisms

## Cancer types (linked)

- [SS](../cancer_types/SS.md) — Sézary syndrome; activity observed in JAK3-mutant cell line model

## Sources
- [PMID:30333627](../papers/30333627.md)

*This page was processed by **wiki-cli** on **2026-05-16**.*
