---
symbol: RRM2B
aliases:
  - p53R2
cancer_types:
  - HCC
tags: []
processed_by: crosslinker
processed_at: 2026-05-21
---

# RRM2B

## Overview

RRM2B (also known as p53R2) encodes ribonucleotide reductase regulatory subunit M2 B, a p53-inducible form of the small subunit of ribonucleotide reductase. It is involved in DNA damage response, replication stress, and mitochondrial DNA synthesis. RRM2B has conflicting reports of tumor-suppressor versus oncogenic roles across cancer types, and has been nominated as a [gemcitabine](../drugs/gemcitabine.md) target in hypoxic conditions.

## Alterations observed in the corpus

- Identified as a cis-driver candidate at the 8q22.3 locus in a 122-patient [HCC](../cancer_types/HCC.md) multi-omics cohort (WES + RNA-seq + proteome + phosphoproteome); described as a p53 target / DNA damage and replication stress regulator with conflicting tumor-suppressor vs oncogenic roles; nominated as a [gemcitabine](../drugs/gemcitabine.md) target in hypoxic conditions [PMID:35508466](../papers/35508466.md)

## Cancer types (linked)

- **[HCC](../cancer_types/HCC.md)**: Cis-driver candidate at 8q22.3 with conflicting tumor-suppressor vs oncogenic roles in a European [HCC](../cancer_types/HCC.md) multi-omics cohort; RRM2B is a p53 target involved in DNA damage response [PMID:35508466](../papers/35508466.md)

## Co-occurrence and mutual exclusivity

- Located at 8q22.3, the same amplification peak as [YWHAZ](../genes/YWHAZ.md), [NUDCD1](../genes/NUDCD1.md), and [ATP6V1C1](../genes/ATP6V1C1.md) in [HCC](../cancer_types/HCC.md) [PMID:35508466](../papers/35508466.md)

## Therapeutic relevance

- Nominated as a potential [gemcitabine](../drugs/gemcitabine.md) target in hypoxic HCC conditions; [gemcitabine](../drugs/gemcitabine.md) may have enhanced efficacy in RRM2B-elevated tumors [PMID:35508466](../papers/35508466.md)

## Open questions

- The conflicting tumor-suppressor vs oncogenic roles of RRM2B across cancer types remain unresolved; functional characterization in HCC specifically is lacking.

## Sources

- [PMID:35508466](../papers/35508466.md)

*This page was processed by **crosslinker** on **2026-05-21**.*
