---
symbol: FN1
aliases: []
cancer_types: []
tags: []
processed_by: entity-page-writer
processed_at: 2026-05-11
---

# FN1

## Overview

FN1 (Fibronectin 1) encodes a large extracellular matrix glycoprotein that mediates cell adhesion, migration, and signaling through integrin receptors. In cancer, FN1 is a key marker of cancer-associated fibroblast (CAF) activation and epithelial-to-mesenchymal transition (EMT), and its upregulation in the tumor stroma is associated with poor prognosis in multiple cancer types.

## Alterations observed in the corpus

- Upregulated as a CAF activation marker in gastric fibroblasts under high-stiffness conditions (16 kPa), downstream of YAP/TAZ mechanotransduction; used as readout of CAF activation alongside ACTA2, CCN2, COL1A1, and COL4A1 in GBC stroma [PMID:40830485](../papers/40830485.md)

## Cancer types (linked)

## Co-occurrence and mutual exclusivity

## Therapeutic relevance

## Open questions

## Sources

- [PMID:40830485](../papers/40830485.md)

*This page was processed by **entity-page-writer** on **2026-05-11**.*
